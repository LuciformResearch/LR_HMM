...
async function main() {
  ...
  const outSqlDir = path.resolve(process.cwd(), 'sql/generated');
  await fs.mkdir(outSqlDir, { recursive: true });
  const dim = Number(getArg('--dim', process.env.EMBEDDING_DIM || '768'));

  const ensureSql = `-- auto-generated by scripts/schema_codegen.ts
...
`;
  ...
  const ensurePath = path.join(outSqlDir, 'ensure.sql');
  await fs.writeFile(ensurePath, ensureSql, 'utf8');
  console.log(`Wrote ${ensurePath}`);
}
